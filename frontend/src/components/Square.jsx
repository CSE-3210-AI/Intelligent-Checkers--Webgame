 ...
 */
const Square = ({ isDark, piece, isHighlighted, isSelected, isMoveable, onClick }) => {
  const bgColor = isDark ? 'bg-[#8B6F47]' : 'bg-[#F0E6D2]';

  return (
    <div
      className={`aspect-square ${bgColor} flex items-center justify-center relative
        ${onClick && isDark ? 'cursor-pointer' : ''}
      `}
      onClick={onClick}
    >
      {/* Green overlay – marks the currently selected piece's square */}
      {isSelected && (
        <div className="absolute inset-0 bg-green-400/40 pointer-events-none z-10" />
      )}
      ...
      {isHighlighted && piece && (
        <div className="absolute inset-0 ring-2 ring-inset ring-yellow-400 pointer-events-none z-20" />
      )}
