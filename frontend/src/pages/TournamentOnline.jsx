import { useState } from 'react';
import { Button } from '@/components/ui/button';
import { useNavigate } from 'react-router-dom';
import AgentPortrait from '../components/AgentPortrait';
import { AGENT_CARD_COPY } from '../lib/agentCards';

export default function TournamentOnline() {
  const [selected, setSelected] = useState('adiba');
  const navigate = useNavigate();

  return (
    <div className="min-h-screen relative flex flex-col items-center justify-center px-4 py-10 sm:py-12">
      <button
        type="button"
        aria-label="Go back"
        className="CyberButton CyberButton--secondary absolute left-4 top-4 flex h-11 items-center gap-1 text-blue-600 hover:text-blue-800 font-medium text-sm z-10 sm:left-8 sm:top-8"
        onClick={() => navigate(-1)}
      >
        Back
      </button>
      <h1 className="cyber-heading text-3xl sm:text-4xl md:text-5xl font-extrabold text-center mb-8 sm:mb-10 bg-linear-to-r from-slate-900 via-blue-800 to-slate-900 bg-clip-text text-transparent">
        Online Benchmark Tournament
      </h1>
      <h2 className="cyber-heading text-xl sm:text-2xl font-bold text-center mb-6 sm:mb-8 text-slate-800">Select Your Internal Agent</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-8 w-full max-w-4xl mb-8 sm:mb-12">
        <div
          className={`cyber-card cyber-agent-card cyber-agent-card--megha rounded-xl p-8 flex flex-col items-center border-2 cursor-pointer transition-all duration-200 ${selected === 'megha' ? 'border-blue-400 ring-2 ring-blue-300/60' : 'border-blue-300/40 hover:border-blue-300/80'}`}
          onClick={() => setSelected('megha')}
        >
          <AgentPortrait profile={{ ...AGENT_CARD_COPY.megha, agentKey: 'megha', type: 'internal_ai' }} side="blue" size="xl" className="mb-4" interactive />
          <h2 className="cyber-heading text-2xl font-bold mb-2 text-blue-100">{AGENT_CARD_COPY.megha.name}</h2>
          <div className="mb-2">
            <span className="cyber-label font-semibold text-sm px-3 py-1 rounded-full">{AGENT_CARD_COPY.megha.badge}</span>
          </div>
          <h3 className="cyber-heading font-semibold text-slate-100 mb-1 mt-4">Strategy Overview</h3>
          <p className="text-slate-300 text-center text-base">{AGENT_CARD_COPY.megha.summary}</p>
        </div>
        <div
          className={`cyber-card cyber-agent-card cyber-agent-card--adiba rounded-xl p-8 flex flex-col items-center border-2 cursor-pointer transition-all duration-200 ${selected === 'adiba' ? 'border-rose-400 ring-2 ring-rose-300/50' : 'border-rose-300/35 hover:border-rose-300/70'}`}
          onClick={() => setSelected('adiba')}
        >
          <AgentPortrait profile={{ ...AGENT_CARD_COPY.adiba, agentKey: 'adiba', type: 'adiba' }} side="red" size="xl" className="mb-4" interactive />
          <h2 className="cyber-heading text-2xl font-bold mb-2 text-rose-100">{AGENT_CARD_COPY.adiba.name}</h2>
          <div className="mb-2">
            <span className="cyber-label font-semibold text-sm px-3 py-1 rounded-full">{AGENT_CARD_COPY.adiba.badge}</span>
          </div>
          <h3 className="cyber-heading font-semibold text-slate-100 mb-1 mt-4">Strategy Overview</h3>
          <p className="text-slate-300 text-center text-base">{AGENT_CARD_COPY.adiba.summary}</p>
        </div>
      </div>
      <Button
        size="lg"
        onClick={() => navigate(`/game?mode=online-benchmark&internalAgent=${selected}`)}
        className="app-action-button"
      >
        Start Benchmark
      </Button>
    </div>
  );
}
